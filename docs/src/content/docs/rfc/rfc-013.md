...
### From Passive Retrieval to Active Learning

Parts A-E describe individual components. Together, they form a **closed-loop
active memory system** where CVX evolves from a retrieval engine into a
learning agent that refines its own memory.
...
| Step | Part | What it provides | Without it |
|------|------|-----------------|------------|
...
**Self-improving**: Each episode refines the memory — wins add knowledge,
fails sharpen the decay. The MDP and Bayesian weights update continuously.

**Non-destructive**: Original expert trajectories are never deleted. Reward
decay reduces their retrieval priority but preserves them for potential
re-evaluation. Clean memory (E7d) ensures fails never pollute the index.

**Compositional**: Each part can be added incrementally. Part A (MDP, Python)
works without Parts B-C. Parts B-C enhance but don't require each other.

**Model-agnostic**: The memory system works with any LLM (qwen:7b, GPT-4o).
Stronger models extract more value from the same memory (E5: 43% with GPT-4o
vs E7b: 27% with qwen:7b, both from same expert trajectories).
