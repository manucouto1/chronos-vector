...
1. `feat/time-partitions` (P0) — `PartitionedTemporalHnsw`, global entity index, parallel fan-out
2. `feat/temporal-lsh` (P1) — T-LSH auxiliary index, multi-probe query, α-adaptive bit allocation
3. `feat/streaming-index` (P2) — WAL + HotBuffer + compaction pipeline
