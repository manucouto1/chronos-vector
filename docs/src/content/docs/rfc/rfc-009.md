...
1. `feat/mcp-server` (P0) — new `cvx-mcp` crate, JSON-RPC over stdio, 8 tool definitions
2. `feat/llm-api-layer` (P0) — composite endpoints (entity_summary, detect_anomalies), summarization middleware
3. `feat/inline-embeddings` (P1) — `Embedder` trait, ONNX backend, API embedding backend
4. `feat/agentic-patterns` (P2) — workflow documentation, prompt templates, examples
