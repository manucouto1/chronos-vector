...
1. `feat/cohort-drift` (P0) — cohort-level drift with all statistics
2. `feat/temporal-join` (P0) — pairwise and group convergence windows
3. `feat/temporal-motifs` (P1) — Matrix Profile motif and discord discovery
4. `feat/granger-embeddings` (P1) — VAR fitting + F-test + direction detection
5. `feat/counterfactual` (P2) — linear extrapolation + Neural ODE (later)
